...
func (rm *room) client(clientID string) (*client, error) {
	...
	rm.clients[clientID] = newClient(clientID, is_initiator, timer)
...
func (rm *room) other_client(clientID string) (*client, error) {
	for otherClientId, otherClient := range rm.clients {
...
func (rm *room) send(srcClientID string, msg string) error {
	...
	log.Printf("Client %s send msg %s in room %s", srcClientID, msg, rm.id)
...
func (rm *room) remove(clientID string) {
	if c, ok := rm.clients[clientID]; ok {
		...
		resp, err := http.Post("http://"+rm.roomSrvUrl+"/bye/"+rm.id+"/"+clientID, "text", nil)
		if err != nil {
